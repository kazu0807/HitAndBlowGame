...
public class HitAndBlow84E {
	...
	public static void main(String[] args) {
		...
		while (guessCount < MAX_GUESS) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if ((input.charAt(i) - '0') >= 0 && (input.charAt(i) - '0') <= 9) {
					...
					// 重複チェック：guessNumbers[0]〜[i-1] に num が含まれているか
					for (int j = 0; j < i; j++) {
						if (guessNumbers[j] == (input.charAt(i) - '0')) {
							System.out.println("数字は重複しないように入力してください。");
							flg = true;
							break;
						}
					}
		...
		if (guessCount >= MAX_GUESS) {
			...
			int answer = 0; // 正解の数字を表示するための変数	
			for (int i = 0; i < secretNumbers.length; i++) {
				answer = answer * 10 + secretNumbers[i]; // 配列の数字を連結して1つの整数にする
			}
